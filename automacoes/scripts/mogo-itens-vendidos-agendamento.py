#!/usr/bin/env python3
"""
BigDog — Mogo "Itens Vendidos por Agendamento" (Mensal)
Roda todo dia 1 e 2 de cada mês, qualquer horário.
Relatório sempre referente ao mês anterior.

Replica: Relatórios > Personalizado > Itens Vendidos por Agendamento
  - Período: primeiro ao último dia do mês anterior

Endpoint: GET /relatorios/BuscaDadosRelatorioDinamico
  idGeradorRelatorios=1 (personalizado!), codRelatorio=0

Campos: A0=Produto, A1=Quantidade
"""
...
print(f"Itens Vendidos por Agendamento: {data_ini} a {data_fim}...")

filtro = f"DataDe{{{data_ini}|DataAte{{{data_fim}|ProdutoEstabelecimento{{"

r_test = session.get(f"{MOGO_URL}/relatorios/BuscaDadosRelatorioDinamico", params={
    'idGeradorRelatorios': '1', 'codRelatorio': '0',
    'filtro': filtro,
...
try:
    todos_registros.sort(key=lambda x: float(str(x.get('A1','0')).replace(',','.')), reverse=True)
except:
    pass
...
for reg in todos_registros:
    try:
        total_qtde += float(str(reg.get('A1','0')).replace(',','.'))
    except:
        pass
...
pasta = '/root/workspaces/cake-brain/relatorios/Mogo/Itens Vendidos Agendamento'
os.makedirs(pasta, exist_ok=True)
xlsx_path = f"{pasta}/{mes_ref}.xlsx"

wb = openpyxl.Workbook()
ws = wb.active
ws.title = "Itens por Agendamento"
...
ws.append([])
ws.append(['TOTAL', f"{total_qtde:,.0f}".replace(',','.')])
...
# ── Salvar JSON ────────────────────────────────────────────────────────────────
registros_nomeados = [
    {'produto': r.get('A0',''), 'quantidade': r.get('A1','')}
...
with open(json_path, 'w', encoding='utf-8') as f:
    json.dump({
        ...
        "total_produtos": len(todos_registros),
        "total_quantidade": f"{total_qtde:,.0f}".replace(',','.'),
        "registros": registros_nomeados
    }, f, ensure_ascii=False, indent=2)
print(f"JSON salvo: {json_path}")

# ── Enviar email ───────────────────────────────────────────────────────────────
top10 = todos_registros[:10]
corpo = f"📅 Itens Vendidos por Agendamento — {mes_nome}\n"
corpo += "=" * 50 + "\n\n"
corpo += f"📅 Período: {data_ini} a {data_fim}\n"
corpo += f"📦 Total de produtos: {len(todos_registros)}\n\n"
corpo += "🏆 Top 10 mais vendidos:\n"
for reg in top10:
    corpo += f"  • {reg.get('A0','')[:35]} — {reg.get('A1','')} un\n"
corpo += "\n(BigDog 🐕)"
...
cmd = [
    ...
    f'--subject "📅 Mogo Itens por Agendamento — {mes_nome} — {len(todos_registros)} produtos" '
    f'--body "{corpo}" '
    f'--attach "{xlsx_path}"'
]
