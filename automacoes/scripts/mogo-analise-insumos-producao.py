#!/usr/bin/env python3
"""
BigDog — Mogo "Análise de Insumos Gastos na Produção" (Mensal)
Roda todo dia 1 e 2 de cada mês, qualquer horário.
Relatório sempre referente ao mês anterior.

Replica: Relatórios > Produção / KDS > Análise de Insumos Gastos na Produção
  - Período: primeiro ao último dia do mês anterior

Endpoint: GET /relatorios/BuscaDadosRelatorioDinamico
  idGeradorRelatorios=0, codRelatorio=99

Campos: A0=Insumo, A1=Qtde, A2=Unidade, A3=Data, A4=Hora,
        A5=Tipo Movimento, A6=Produto Produzido, A7=Chave
"""
...
print(f"Análise de Insumos Gastos na Produção: {data_ini} a {data_fim}...")

filtro = f"DataDe{{{data_ini}|DataAte{{{data_fim}|ProdutoEstabelecimento{{|SubCategoria{{|Categoria{{|ProdutoEstabelecimentoProd{{"
...
pasta = '/root/workspaces/cake-brain/relatorios/Mogo/Analise Insumos Producao'
os.makedirs(pasta, exist_ok=True)
xlsx_path = f"{pasta}/{mes_ref}.xlsx"

wb = openpyxl.Workbook()
ws = wb.active
ws.title = "Insumos Produção"
...
for i, w in enumerate([12, 10, 40, 40, 10, 8, 20], 1):
...
registros_nomeados = [
    {
        'data':              r.get('A3',''),
        'hora':              r.get('A4',''),
        'produto_produzido': r.get('A6',''),
        'insumo':            r.get('A0',''),
        'qtde':              r.get('A1',''),
        'unidade':           r.get('A2',''),
        'tipo_movimento':    r.get('A5',''),
    }
...
# ── Enviar email ───────────────────────────────────────────────────────────────
corpo = f"🧪 Insumos Gastos na Produção — {mes_nome}\n"
corpo += "=" * 50 + "\n\n"
...
cmd = [
    ...
    f'--subject "🧪 Mogo Insumos na Produção — {mes_nome} — {len(todos_registros)} registros" '
    f'--body "{corpo}" '
    f'--attach "{xlsx_path}" '
    f'--attach "{json_path}"'
]
