#!/usr/bin/env python3
"""
BigDog — Mogo "Descontos Concedidos" (Mensal)
Roda todo dia 1 e 2 de cada mês, qualquer horário.
Relatório sempre referente ao mês anterior.

Replica: Relatórios > Financeiro > Descontos Concedidos
  - Período: primeiro ao último dia do mês anterior
  - Sem filtros adicionais

Endpoint: GET /relatorios/BuscaDadosRelatorioDinamico
  idGeradorRelatorios=0, codRelatorio=45

Campos: A0=Atendente, A1=Cliente, A2=Desconto, A3=Data, A4=Hora,
        A5=Origem, A6=Tipo Pedido
"""
...
print(f"Descontos Concedidos: {data_ini} a {data_fim}...")

filtro = f"Atendente{{|DataDe{{{data_ini}|OrigemPedido{{|DataAte{{{data_fim}|MotivoDesconto{{"
...
for reg in todos_registros:
    try:
        v = str(reg.get('A2', '0')).replace('.','').replace(',','.').strip()
        total_desconto += float(v)
    except:
        pass
total_fmt = f"R$ {total_desconto:,.2f}".replace(',','X').replace('.', ',').replace('X','.')
...
pasta = '/root/workspaces/cake-brain/relatorios/Mogo/Descontos Concedidos'
os.makedirs(pasta, exist_ok=True)
xlsx_path = f"{pasta}/{mes_ref}.xlsx"

wb = openpyxl.Workbook()
ws = wb.active
ws.title = "Descontos Concedidos"
...
for i, w in enumerate([12, 8, 35, 14, 14, 15, 20], 1):
...
cmd = [
    ...
    f'--subject "🏷️ Mogo Descontos Concedidos — {mes_nome} — {total_fmt}" '
    f'--body "{corpo}" '
    f'--attach "{xlsx_path}" '
    f'--attach "{json_path}"'
]
