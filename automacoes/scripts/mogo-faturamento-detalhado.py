...
r_test = session.get(f"{MOGO_URL}/relatorios/BuscaDadosRelatorioDinamico", params={
    ...
    'codRelatorio': '58',
    'filtro': filtro,
    'gridparamns': json.dumps({"Searching": True, "RecordsCount": 1, "PageIndex": 0,
                                "SortingName": "", "SortingOrder": "ASC"}),
    'colunas': '[]',
    'dbNameFranquia': ''
}, timeout=30)

total_records = r_test.json().get('records', 0)
print(f"Total de registros: {total_records}")

# Buscar tudo de uma vez (em lotes de 5000 se necessário)
todos_registros = []
lote = 5000
pagina = 0
...
while True:
    ...
    if len(rows) < lote or len(todos_registros) >= int(total_records):
        break
    pagina += 1
